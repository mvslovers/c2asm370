...
print_mem_expr (FILE *outfile, tree expr)
{
	if (TREE_CODE (expr) == COMPONENT_REF) {
		...
		if (DECL_NAME (TREE_OPERAND (expr, 1))) {
			fprintf (outfile, ".%s", IDENTIFIER_POINTER (DECL_NAME (TREE_OPERAND (expr, 1))));
		}
	}
...
int
format_mem_expr (char *buf, int bufsize, tree expr)
{
	int	len = 0;
	
	if (bufsize <= 0) goto quit;
	
	if (TREE_CODE (expr) == COMPONENT_REF) {
		if (TREE_OPERAND (expr, 0)) {
			len = format_mem_expr (buf, bufsize, TREE_OPERAND (expr, 0));
		}
		else {
			len = snprintf(buf, bufsize, "<variable>");
		}
		...
		len += format_mem_expr (&buf[len], bufsize-len, TREE_OPERAND (expr, 0));
		len += snprintf(&buf[len], bufsize-len, ")");
	}
	else if (DECL_NAME (expr)) {
		len = snprintf(buf, bufsize, "%s", IDENTIFIER_POINTER (DECL_NAME (expr)));
	}
	else if (TREE_CODE (expr) == RESULT_DECL) {
		len = snprintf(buf, bufsize, "<result>");
	}
	else {
		len = snprintf(buf, bufsize, "<anonymous>");
	}
...
print_rtx (rtx in_rtx)
{
	...
	if (GET_CODE (in_rtx) > NUM_RTX_CODE) {
	...
	if (dump_for_graph && (is_insn || GET_CODE (in_rtx) == NOTE || 
		...
		if (! flag_simple) {
			if (in_rtx->in_struct) fputs ("/s", outfile);

			if (in_rtx->volatil) fputs ("/v", outfile);
	  
			if (in_rtx->unchanging) fputs ("/u", outfile);
	  
			if (in_rtx->integrated) fputs ("/i", outfile);
	  
			if (in_rtx->frame_related) fputs ("/f", outfile);
	  
			if (in_rtx->jump) fputs ("/j", outfile);
	  
			if (in_rtx->call) fputs ("/c", outfile);
	...
	for (; i < GET_RTX_LENGTH (GET_CODE (in_rtx)); i++) {
		switch (*format_ptr++) {
			...
			/* 0 indicates a field for internal use that should not be 
			 * printed. An exception is the third field of a NOTE, 
			 * where it indicates that the field has several different 
			 * valid contents.  */
			case '0':
				...
				if (i == 3 && GET_CODE (in_rtx) == NOTE) {
					switch (NOTE_LINE_NUMBER (in_rtx)) {
						...
						case NOTE_INSN_BASIC_BLOCK:	{
							...
							if (bb != 0) fprintf (outfile, " [bb %d]", bb->index);
							break;
						}
			...
			case 'V':
				...
				if (NULL != XVEC (in_rtx, i)) {
					indent += 2;
					if (XVECLEN (in_rtx, i)) sawclose = 1;
			...
			case 'w':
				if (! flag_simple) fprintf (outfile, " ");
			...
			case 'i':
				if (i == 5 && GET_CODE (in_rtx) == NOTE) {
					/* This field is only used for NOTE_INSN_DELETED_LABEL, 
					 * and other times often contains garbage from 
					 * INSN->NOTE death.  */
					...
					else if (flag_dump_unnumbered && (is_insn || 
						GET_CODE (in_rtx) == NOTE))
						fputc ('#', outfile);
					else
