...
assemble_start_function (tree decl, const char *fnname)
{
	...
	if (TARGET_CONSTANTS_FIRST) output_constant_pool (fnname, decl);
...
static void
asm_emit_uninitialised(tree decl, const char *name, int size, int rounded)
{
	enum {
		asm_dest_common,
		asm_dest_bss,
...
void
output_constant_pool (const char *fnname, tree fndecl ATTRIBUTE_UNUSED)
{
	struct pool_constant *pool;
	rtx x;
	union real_extract u;
	...
	for (pool = first_pool; pool; pool = pool->next) {
		...
		switch (GET_MODE_CLASS (pool->mode)) {
			...
			case MODE_VECTOR_FLOAT:
			{
				...
				if (GET_CODE (x) != CONST_VECTOR) abort ();
