...
static TS target_switches [] = TARGET_SWITCHES;
...
static TO target_options [] = TARGET_OPTIONS;
#endif

...
display_target_options ()
{
	...
	if (ARRAY_SIZE (target_switches) > 1
		...
		for (i = ARRAY_SIZE (target_switches); i--;) {
			TS 			*ts 		= &target_switches[i];
			const char 	*option     = ts->name;
			const char 	*description= ts->description;
		...
		for (i = ARRAY_SIZE (target_options); i--;)	{
			TO 			*to			= &target_options[i];
			const char 	*option     = to->prefix;
			const char  *description= to->description;
...
decode_f_option (const char *arg)
{
	...
	if (!strcmp (arg, "fast-math"))
		...
		int val = read_integral_parameter (option_value, arg - 2, MAX_INLINE_INSNS);
		...
		align_functions = read_integral_parameter (option_value, arg - 2, align_functions);
		...
		align_labels = read_integral_parameter (option_value, arg - 2, align_labels);
	else if ((option_value = skip_leading_substring (arg, "stack-limit-register="))) {
	...
	else if ((option_value = skip_leading_substring (arg, "stack-limit-symbol="))) {
		const char *nm;

		nm = ggc_strdup (option_value);
		stack_limit_rtx = gen_rtx_SYMBOL_REF (Pmode, nm);
	}
	else if ((option_value = skip_leading_substring (arg, "message-length=")))
		output_set_maximum_length(&global_dc->buffer, read_integral_parameter
			(option_value, arg - 2, diagnostic_line_cutoff (global_dc)));
	else if ((option_value = skip_leading_substring (arg, "diagnostics-show-location="))) {
...
set_target_switch (const char *name)
{
	...
	int no = 0;
	const char *flag_name = name;
	
	if (strncmp(flag_name, "no-", 3) == 0) {
		no = 1;
		flag_name += 3;
	}

	for (j = 0; j < ARRAY_SIZE (target_switches); j++) {
		TS *ts = &target_switches[j];

		if (strcmp(ts->name, flag_name)) continue;

		/* we found the flag name */
		if (no) {
			/* "no-" flag option, clear the flag */
			target_flags &= ~ ts->value;
		}
		else {
			target_flags |= ts->value;
		}
	...
	if (!valid_target_option) {
		for (j = 0; j < ARRAY_SIZE (target_options); j++) {
			TO *to = &target_options[j];
			int len = strlen (to->prefix);
			
			if (!strncmp (to->prefix, name, len)) {
				*(to->variable) = name + len;
				valid_target_option = 1;
			}
		}
	}
...
print_single_switch (FILE *file, int pos, int max, 
	const char *indent, const char *sep, const char *term, const char *type, const char *name)
{
	/* The ultrix fprintf returns 0 on success, so compute the result 
	** we want here since we need it for the following test.  
	*/
...
static void
print_switch_values (FILE *file, int pos, int max, const char *indent, const char *sep, const char *term)
{
	size_t 	j;
	char 	**p;
	...
	for (p = &save_argv[1]; *p != NULL; p++)
		if (**p == '-') {
			...
			if (strcmp (*p, "-quiet") == 0) continue;

			if (strcmp (*p, "-version") == 0) continue;
	...
	if (pos > 0) fprintf (file, "%s", term);
	fprintf(file, "%s %s", indent, term);
	...
	*/
	...
	for (j = 0; j < ARRAY_SIZE (target_switches); j++) {
		...
		if (ts->name[0] != '\0' && ts->value > 0) {
			pos = print_single_switch (file, pos, max, indent, sep, term,
			   ((ts->value & target_flags) == ts->value) ? "-m" : "-mno-", 
			   ts->name);
		}
	}
...
init_asm_output (const char *name)
{
	...
	if (!flag_syntax_only) {
		...
		bp = realpath(main_input_filename, NULL);
		if (bp) {
			fprintf(asm_out_file, " TITLE '%.60s'\n", bp);
			free(bp);
		}
		else {
			fprintf(asm_out_file, " TITLE '%.60s'\n", main_input_filename);
		...
		if (flag_verbose_asm) {
			...
			print_switch_values (asm_out_file, 0, MAX_LINE,
			       "*", " ", "\n");
			/* Add a blank line here so it appears in assembler output 
			** but not screen output.  */
			fprintf (asm_out_file, "*\n");
		}
	}
}
		...
		if (lang_processed || indep_processed)
			...
			if (j != ARRAY_SIZE (documented_lang_options)) {
				if (extra_warnings)	{
					...
					if (lang)
						warning ("(it is valid for %s but not the selected language)",
							lang);
				}
			}
...
process_options ()
{
	...
	if (!TARGET_ALLOWS_PROFILING_WITHOUT_FRAME_POINTER
		&& profile_flag && flag_omit_frame_pointer) {
