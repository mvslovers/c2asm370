		...
		if (expr) {
			if (!wrote) {
				fprintf (asm_out_file, "\t%s %s",
					ASM_COMMENT_START, addressp ? "*" : "");
				wrote = 1;
			}
			else {
				/* continue expression operands to next line */
...
extern int format_mem_expr (char *buf, int bufsize, tree expr);
...
output_asm_operand_names (rtx *operands, int *oporder, int nops)
{
	...
	for (i = 0; i < nops; i++) {
		expr = get_mem_expr_from_op (operands[oporder[i]], &addressp);
		if (expr) {
			if (i > 0 && len > 0) {
				char	tbuf[256] = "";
				int		tlen = format_mem_expr(tbuf, sizeof(tbuf), expr);
				
				if (!tlen) continue;
				
				/* check for same name formatted again */
				if (strcmp(buf, tbuf)==0) continue;
				
				if (tbuf[0]=='.') {
					len += snprintf(&buf[len], buflen-len, ",%s", &tbuf[1]);
					continue;
				}
				len += snprintf(&buf[len], buflen-len, ",%s", tbuf);
				continue;
			}
			len += format_mem_expr(&buf[len], buflen-len, expr);
		}
	}
